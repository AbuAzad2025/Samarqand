...
export default function ControlProjects() {
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      const res = await createAdminProject({ title, slug: slug || undefined });
      if (res.ok && res.id) {
        setTitle("");
        setSlug("");
        navigate(`/control/projects/${res.id}`);
      }
    } finally {
      setCreating(false);
    }
  ...
  return (
    <div>
      <div className="flex items-end justify-between gap-4 flex-wrap mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {items.length} مشروع • {liveCount} منشور
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-8">
        <div className="text-lg font-bold text-gray-900 mb-4">إضافة مشروع</div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="divide-y">
            {items.map((p) => (
              <div key={p.id} className="p-6 flex items-center gap-5">
            ...
            {!items.length && <div className="p-6 text-gray-600">لا يوجد مشاريع.</div>}
          </div>
        )}
      </div>
    </div>
